...
func NewPath() Path {
	return Path(make([]util.MicroPoint, 0))
}
func NewPaths() Paths {
...
func Int128Mul(lhs, rhs util.Micrometer) *big.Int {
...
type IntRect struct {
	left, top, right, bottom util.Micrometer
}

func NewIntRect(l, t, r, b util.Micrometer) *IntRect {
	this := new(IntRect)
	this.left = l
	this.top = t
	this.right = r
...
type TEdge struct {
	Bot, Curr, Top, Delta            util.MicroPoint
	Dx                               float64
	PolyTyp                          PolyType
	Side                             EdgeSide
...
func (c *ClipperBase) Swap(val1, val2 *util.Micrometer) {
	*val1, *val2 = *val2, *val1
}
...
func (c *ClipperBase) PointIsVertex(pt util.MicroPoint, pp *OutPt) bool {
	pp2 := pp
	for {
		if pp2.Pt == pt {
			return true
...
func (c *ClipperBase) PointOnLineSegment(pt,
	linePt1, linePt2 util.MicroPoint, UseFullRange bool) bool {
	if UseFullRange {
...
func (c *ClipperBase) PointOnPolygon(pt util.MicroPoint, pp *OutPt, UseFullRange bool) bool {
	pp2 := pp
	for {
...
func (c *ClipperBase) SlopesEqual3(pt1, pt2,
	pt3 util.MicroPoint, UseFullRange bool) bool {
	if UseFullRange {
...
func (c *ClipperBase) SlopesEqual4(pt1, pt2,
	pt3, pt4 util.MicroPoint, UseFullRange bool) bool {
	if UseFullRange {
...
func (c *ClipperBase) RangeTest(Pt util.MicroPoint, useFullRange *bool) {
	if *useFullRange {
		if Pt.X() > util.MaxMicrometer || Pt.Y() > util.MaxMicrometer || -Pt.X() > util.MaxMicrometer || -Pt.Y() > util.MaxMicrometer {
			panic(NewClipperException("Coordinate outside allowed range"))
		} else if Pt.X() > util.MinMicrometer || Pt.Y() > util.MinMicrometer || -Pt.X() > util.MinMicrometer || -Pt.Y() > util.MinMicrometer {
			*useFullRange = true
...
func (c *ClipperBase) InitEdge(e, eNext, ePrev *TEdge, pt util.MicroPoint) {
	e.Next = eNext
	e.Prev = ePrev
	e.Curr = pt.Copy()
	e.OutIdx = Unassigned
...
func (c *ClipperBase) Pt2IsBetweenPt1AndPt3(pt1, pt2, pt3 util.MicroPoint) bool {
...
func (c *ClipperBase) SetDx(e *TEdge) {
	e.Delta = util.NewMicroPoint(e.Top.X()-e.Bot.X(), e.Top.Y()-e.Bot.Y())
	if e.Delta.Y() == 0 {
		e.Dx = horizontal
	} else {
...
type TZFillCallback interface {
	ZFill(bot1, top1, bot2, top2, intersectPt util.MicroPoint)
}
...
func (c *Clipper) InsertScanbeam(Y util.Micrometer) {
	if c.m_Scanbeam == nil {
...
func (c *Clipper) PopScanbeam() util.Micrometer {
	Y := c.m_Scanbeam.Y
...
func (c *Clipper) AddJoin(Op1, Op2 *OutPt, OffPt util.MicroPoint) {
	j := new(Join)
	j.OutPt1 = Op1
	j.OutPt2 = Op2
	j.OffPt = OffPt
...
func (c *Clipper) AddGhostJoin(Op *OutPt, OffPt util.MicroPoint) {
	j := new(Join)
	j.OutPt1 = Op
	j.OffPt = OffPt
...
func (c *Clipper) InsertLocalMinimaIntoAEL(botY util.Micrometer) {
...
func (c *Clipper) AddLocalMaxPoly(e1, e2 *TEdge, pt util.MicroPoint) {
	c.AddOutPt(e1, pt)
	if e2.WindDelta == 0 {
		c.AddOutPt(e2, pt)
	}
...
func (c *Clipper) AddLocalMinPoly(e1, e2 *TEdge, pt util.MicroPoint) *OutPt {
	var result *OutPt
	var e, prevE *TEdge
...
func (c *Clipper) AddOutPt(e *TEdge, pt util.MicroPoint) *OutPt {
...
func (c *Clipper) HorzSegmentsOverlap(seg1a, seg1b, seg2a, seg2b util.Micrometer) bool {
	if seg1a > seg1b {
		c.Swap(&seg1a, &seg1b)
	}
	if seg2a > seg2b {
...
func (c *Clipper) GetDx(pt1, pt2 util.MicroPoint) float64 {
	if pt1.Y() == pt2.Y() {
		return horizontal
	} else {
...
func (c *Clipper) IntersectEdges(e1, e2 *TEdge, pt util.MicroPoint, protect bool) {
...
func (c *Clipper) GetHorzDirection(HorzEdge *TEdge, Dir *Direction, Left, Right *util.Micrometer) {
...
func (c *Clipper) ProcessHorizontal(horzEdge *TEdge, isTopOfScanbeam bool) {
	var dir Direction
	var horzLeft, horzRight util.Micrometer
		...
		for e != nil {
			...
			if (dir == DLeftToRight && e.Curr.X() <= horzRight) ||
				...
				if e == eMaxPair && IsLastHorz {
					...
					Pt := util.NewMicroPoint(e.Curr.X(), horzEdge.Curr.Y())
...
func (c *Clipper) IsMaxima(e *TEdge, Y util.Micrometer) bool {
	return (e != nil && e.Top.Y() == Y && e.NextInLML == nil)
}

//------------------------------------------------------------------------------

func (c *Clipper) IsIntermediate(e *TEdge, Y util.Micrometer) bool {
...
func (c *Clipper) ProcessIntersections(topY util.Micrometer) bool {
	if c.m_ActiveEdges == nil {
		return true
	}
	//	defer func() {
...
func (c *Clipper) BuildIntersectList(topY util.Micrometer) {
	if c.m_ActiveEdges == nil {
		return
	}
...
func Round(value float64) util.Micrometer {
	if value < 0 {
		return util.Micrometer(value - 0.5)
	} else {
		return util.Micrometer(value + 0.5)
	}
}
...
func TopX(edge *TEdge, currentY util.Micrometer) util.Micrometer {
...
func (c *Clipper) IntersectPoint(edge1, edge2 *TEdge) (ip util.MicroPoint) {
	var b1, b2 float64
	//nb: with very large coordinate values, it's possible for SlopesEqual() to
	//return false but for the edge.Dx value be equal due to float64 precision rounding.
	if edge1.Dx == edge2.Dx {
		ip = util.NewMicroPoint(edge1.Curr.Y(), TopX(edge1, ip.Y()))
		return
	}
	...
	if edge1.Delta.X() == 0 {
		...
		if c.IsHorizontal(edge2) {
			...
			ip.SetY(Round(float64(ip.X())/edge2.Dx + b2))
		}
		...
		if c.IsHorizontal(edge1) {
			...
			ip.SetY(Round(float64(ip.X())/edge1.Dx + b1))
		}
	} else {
		...
		if math.Abs(edge1.Dx) < math.Abs(edge2.Dx) {
			ip.SetX(Round(edge1.Dx*q + b1))
		} else {
			ip.SetX(Round(edge2.Dx*q + b2))
		}
	}
	...
	if ip.Y() < edge1.Top.Y() || ip.Y() < edge2.Top.Y() {
		...
		if math.Abs(edge1.Dx) < math.Abs(edge2.Dx) {
			ip.SetX(TopX(edge1, ip.Y()))
		} else {
			ip.SetX(TopX(edge2, ip.Y()))
		}
	}
	//finally, don't allow 'ip' to be BELOW curr.Y() (ie bottom of scanbeam) ...
	if ip.Y() > edge1.Curr.Y() {
		ip.SetY(edge1.Curr.Y())
...
func (c *Clipper) ProcessEdgesAtTopOfScanbeam(topY util.Micrometer) {
	e := c.m_ActiveEdges
	for e != nil {
...
func (c *Clipper) BuildResult() Paths {
	...
	for i := 0; i < len(c.m_PolyOuts); i++ {
		...
		pg := Path(make([]util.MicroPoint, 0, cnt))
		for j := 0; j < cnt; j++ {
			pg = append(pg, p.Pt)
			p = p.Prev
		}
...
func (c *Clipper) BuildResult2(polytree *PolyTree) {
	...
	for i := 0; i < len(c.m_PolyOuts); i++ {
		...
		outRec.PolyNode = pn
		pn.m_polygon = make([]util.MicroPoint, cnt)
		op := outRec.Pts.Prev
		for j := 0; j < cnt; j++ {
...
func (c *Clipper) GetOverlap(a1, a2, b1, b2 util.Micrometer, Left, Right *util.Micrometer) bool {
	if a1 < a2 {
		if b1 < b2 {
			*Left = max(a1, b1)
			*Right = min(a2, b2)
...
func (c *Clipper) JoinHorz(op1, op1b, op2, op2b *OutPt,
	Pt util.MicroPoint, DiscardLeft bool) bool {
	var Dir1 Direction
...
func (c *Clipper) JoinPoints(j *Join, outRec1, outRec2 *OutRec) bool {
	...
	if isHorizontal && (j.OffPt == j.OutPt1.Pt) && (j.OffPt == j.OutPt2.Pt) {
		...
		var Left, Right util.Micrometer
...
func PointInPolygon(pt util.MicroPoint, path Path) int {
	result := 0
	cnt := len(path)
	if cnt < 3 {
		return 0
...
func (c *Clipper) pointInPolygon(pt util.MicroPoint, op *OutPt) int {
	//returns 0 if false, +1 if true, -1 if pt ON polygon boundary
	//See "The Point in Polygon Problem for Arbitrary Polygons" by Hormann & Agathos
	//http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.88.5498&rep=rep1&type=pdf
	result := 0
...
func (c *Clipper) DistanceSqrd(pt1, pt2 util.MicroPoint) float64 {
...
func (c *Clipper) DistanceFromLineSqrd(pt, ln1, ln2 util.MicroPoint) float64 {
...
func (c *Clipper) SlopesNearCollinear(pt1,
	pt2, pt3 util.MicroPoint, distSqrd float64) bool {
	return c.DistanceFromLineSqrd(pt2, pt1, pt3) < distSqrd
}

//------------------------------------------------------------------------------

func (c *Clipper) PointsAreClose(pt1, pt2 util.MicroPoint, distSqrd float64) bool {
	dx := float64(pt1.X() - pt2.X())
	dy := float64(pt1.Y() - pt2.Y())
...
func (c *Clipper) CleanPolygon(path Path, distance float64) Path {
	...
	result := Path(make([]util.MicroPoint, cnt))
	for i := 0; i < cnt; i++ {
		result[i] = op.Pt
		op = op.Next
	}
...
func (c *Clipper) Minkowski(pattern, path Path, IsSum, IsClosed bool) Paths {
	...
	if IsSum {
		for i := 0; i < pathCnt; i++ {
			result[i] = make([]util.MicroPoint, polyCnt)
	...
	for i := 0; i < pathCnt-1+delta; i++ {
		for j := 0; j < polyCnt; j++ {
			quad := Path(make([]util.MicroPoint, 4))
...
func (c *Clipper) TranslatePath(path Path, delta util.MicroPoint) Path {
	outPath := Path(make([]util.MicroPoint, len(path)))
	for i := 0; i < len(path); i++ {
		outPath[i] = util.NewMicroPoint(path[i].X()+delta.X(), path[i].Y()+delta.Y())
	}
	return outPath
}
...
func NewClipperOffset() *ClipperOffset {
	...
	co.m_lowest = util.NewMicroPoint(-1, 0)
	return co
}
...
func (co *ClipperOffset) Round(value float64) util.Micrometer {
	if value < 0 {
		return util.Micrometer(value - 0.5)
	} else {
		return util.Micrometer(value + 0.5)
...
func (co *ClipperOffset) AddPath(path Path, joinType JoinType, endType EndType) {
	...
	newNode.m_polygon = Path(make([]util.MicroPoint, 0, highI+1))
	...
	if co.m_lowest.X() < 0 {
		co.m_lowest = util.NewMicroPoint(util.Micrometer(co.m_polyNodes.ChildCount()-1), util.Micrometer(k))
	} else {
...
func (co *ClipperOffset) GetUnitNormal(pt1, pt2 util.MicroPoint) *DoublePoint {
...
func (co *ClipperOffset) DoOffset(delta float64) {
	...
	for i := 0; i < co.m_polyNodes.ChildCount(); i++ {
		...
		co.m_destPoly = Path(make([]util.MicroPoint, 0, int(steps)+4))
		...
		if length == 1 {
			if node.m_jointype == JtRound {
				...
				for j := 1; j <= int(steps); j++ {
					co.m_destPoly = append(co.m_destPoly, util.NewMicroPoint(
		...
		if node.m_endtype == EtClosedPolygon {
			...
			co.m_destPolys = append(co.m_destPolys, co.m_destPoly)
			co.m_destPoly = Path(make([]util.MicroPoint, 0))
			//re-build m_normals ...
...
func (co *ClipperOffset) Execute(delta float64) (solution Paths) {
	...
	if delta > 0 {
		...
		outer := Path(make([]util.MicroPoint, 4))
